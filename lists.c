...
check_elo(const char *elo_string, StringArray *list)
{
    unsigned list_index;
    unsigned game_elo;
    Boolean wanted_arr[list->num_used_elements];
    unsigned int i;
    for (i = 0; i < list->num_used_elements; i++)
    {
        wanted_arr[i] = FALSE;
	...
	for (list_index = 0; (list_index < list->num_used_elements); list_index++) {
		...
		if ((sscanf(list_string, "%u", &list_elo) == 1)) {
		    switch (operator) {
			case LESS_THAN:
                wanted_arr[list_index] = game_elo < list_elo;
			    break;
			case LESS_THAN_OR_EQUAL_TO:
                wanted_arr[list_index] = game_elo <= list_elo;
			    break;
    ...
    unsigned arr_iter;
    for (arr_iter = 0; arr_iter < list->num_used_elements; arr_iter++)
    {
        if (!wanted_arr[arr_iter])
        {
            break;
        }
    }
    Boolean wanted_arr_all = (arr_iter == list->num_used_elements);
    return wanted_arr_all;
}
